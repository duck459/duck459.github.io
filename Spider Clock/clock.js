...
function getTzTime(tz) {
    try {
        const now = new Date();
        const options = { timeZone: tz, hour: 'numeric', minute: 'numeric', second: 'numeric', hourCycle: 'h23' };
...
// Mapping of continents to popular IANA timezone cities
const timezoneData = {
    Africa: ["Cairo", "Johannesburg", "Lagos", "Nairobi"],
    America: ["New_York", "Los_Angeles", "Chicago", "Sao_Paulo", "Toronto", "Mexico_City"],
    Asia: ["Tokyo", "Shanghai", "Dubai", "Kolkata", "Singapore", "Seoul"],
    Australia: ["Sydney", "Melbourne", "Brisbane", "Perth"],
    Europe: ["London", "Paris", "Berlin", "Rome", "Moscow", "Madrid"]
};
...
// Function to populate the city dropdown based on selected continent
function populateCities(continent) {
    citySelect.innerHTML = '<option value="">-- Select a City --</option>';
    
    if (continent) {
        citySelect.disabled = false;
        timezoneData[continent].forEach(city => {
...
// Function to start the clock for a specific timezone
function startClock(timezone) {
    clearInterval(clockInterval);
    if (timezone) {
        currentTimezone = timezone;
        updateTime(timezone); // Run immediately
        clockInterval = setInterval(() => updateTime(timezone), 1000);
    } else {
        timeDisplay.textContent = "00:00:00";
    }
}
...
// When user manually changes Continent
...
// --- INITIALIZATION (Set Asia as Default) ---

function initDefaultClock() {
    const defaultContinent = "Asia";
    const defaultCity = "Tokyo"; // You can change this to "Kolkata", "Dubai", etc.
    
    // 1. Set the continent dropdown visually to "Asia"
    continentSelect.value = defaultContinent;
    
    // 2. Populate the city dropdown with Asia's cities
    populateCities(defaultContinent);
    
    // 3. Set the city dropdown visually to the default city
    citySelect.value = `${defaultContinent}/${defaultCity}`;
    
    // 4. Start the live clock for that city
    startClock(`${defaultContinent}/${defaultCity}`);
}
